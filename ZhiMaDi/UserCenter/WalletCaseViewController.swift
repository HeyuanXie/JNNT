...
import UIKit
// 提现
class WalletCaseViewController: UIViewController,UITableViewDataSource, UITableViewDelegate,ZMDInterceptorProtocol,ZMDInterceptorMoreProtocol {
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        if indexPath.section == 0{
            ...
            self.amountTextField.placeholder = "请输入提现金额"
    ...
    func updateUI() {
        ...
        let label = ZMDTool.getLabel(CGRectMake(12, 14, kScreenWidth - 24,16), text: "(提示：提现金额不能低于100元)", fontSize: 16)
        footView.addSubview(label)
        let btn = ZMDTool.getButton(CGRect(x: 12, y: 102, width: kScreenWidth - 24, height: 50), textForNormal: "申请提现", fontSize: 20, textColorForNormal: UIColor.whiteColor(), backgroundColor: RGB(235,61,61,1.0)) { (sender) -> Void in
        ...
        headView.backgroundColor = UIColor.clearColor()
        let enabelLbl = ZMDTool.getLabel(CGRectMake(12, 22, kScreenWidth - 24,16), text: "", fontSize: 16)
        headView.addSubview(enabelLbl)
        enabelLbl.attributedText = "可提现金额：1500".AttributedText("1500", color: RGB(235,61,61,1.0))
...
//        cardlabel.attributedText = "中国工商银行 储蓄卡 尾号0425".AttributedText("储蓄卡 尾号0425", color: UIColor.whiteColor())
        ...
        for _ in [50,110] {
            i++
            let line = ZMDTool.getLine(CGRect(x: 0, y: 50, width: kScreenWidth, height: 0.5))
